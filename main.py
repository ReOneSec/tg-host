import os import logging import tempfile import zipfile import re from datetime import datetime from collections import defaultdict from dotenv import load_dotenv
...
In-memory storage (consider using Firestore for production)
...
try:
    if data == 'upload':
        ...
        buttons = [
            [InlineKeyboardButton(f"🗑️ {f['name']}", callback_data=f"delete_{i}")]
...
if name == "main": app = ApplicationBuilder().token(os.getenv("BOT_TOKEN")).build() app.add_handler(CommandHandler("start", start)) app.add_handler(MessageHandler(filters.Document.ALL, handle_file)) app.add_handler(CallbackQueryHandler(button_handler))
